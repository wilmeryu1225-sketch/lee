...
VOID LeechSvc_Interactive(_In_ PLEECHSVC_CONFIG pConfig)
{
    ...
    if(pConfig->fgRPC) {
        if(!RpcStartGRPC(pConfig)) {
            MessageBoxA(NULL, "Unable to start GRPC service.", "", MB_OK | MB_ICONHAND);
            RpcStop();
            LeechRpcOnUnloadClose();
            return;
        }
    ...
    if(pConfig->fMSRPC) {
        ...
        if(FAILED(status)) {
            MessageBoxA(NULL, "Unable to start MSRPC service", "", MB_OK | MB_ICONHAND);
            RpcStop();
            LeechSvc_ReportSvcStatus(SERVICE_STOPPED, status, 0);
            return;
    ...
    if(!status)MessageBoxA(NULL, "Initialized successfully", "", MB_OK);
